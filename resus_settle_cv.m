function dcdt = resus_settle_cv(t,c)
% kernal for settling/diffusion model
% This uses control volume formulation
% ws is a uniform scalar, K is on interfaces
% dz is array of cell-center locations

% csherwood@usgs.gov

% TODO - use of dz is inconsistent...should use dzc (centers) for diffusion
%        and dzf (faces) for flux differences. This version works for 
%        uniform grids with length(dzc) = length(c)

global dz w K Erate Drate
nz = length(c);
dcdt = zeros(nz,1);
% should use dzc here
dcdt(1) =      K(1)*(c(2)-c(1))/dz(1);
dcdt(2:nz-1) = K(2:nz-1).*(c(3:nz)-c(2:nz-1))./dz(2:nz-1)...
              -K(1:nz-2).*(c(2:nz-1)-c(1:nz-2))./dz(1:nz-2);
dcdt(nz) =     K(nz-1)*(c(nz-1)-c(nz))/dz(nz-1);

% upwind flux 
dcdt(2:nz) =   dcdt(2:nz)   + (w>0)*abs(w)*c(1:nz-1) - (w<0)*abs(w)*c(2:nz);
dcdt(1:nz-1) = dcdt(1:nz-1) - (w>0)*abs(w)*c(1:nz-1) + (w<0)*abs(w)*c(2:nz);

% settling and resuspension
dcdt(1) = dcdt(1)-Drate*(w<0)*abs(w)*c(1) + Erate;
dcdt = dcdt./dz; % dzf here
